...
namespace Wini.SaleMultipleChannel.Lazop
{
    ...
    public class LazopLogger : IDisposable, ILazopLogger
    {
        ...
        private string _logDirectory = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + "/logs/";
        ...
        private void FileWrite(Msg msg)
        {
            try
            {
                ...
                if (_writer != null)
                {
                    ...
                    _fileSize += Encoding.UTF8.GetBytes(msg.ToString()).Length;
                    _writer.Flush();
                }
            }
            catch (Exception e)
        ...
        public void TraceApiError(string appKey, string sdkVersion, string apiName, string url, Dictionary<string, string> parameters, double latency, string errorMessage)
        {
        ...
        {
            LogWrite(message, MsgLevel.Error);
        }

        public void Error(string format, params object[] args)
        {
            LogWrite(string.Format(format, args), MsgLevel.Error);
        }

        public void Warn(string message)
        {
            LogWrite(message, MsgLevel.Warn);
        }

        public void Warn(string format, params object[] args)
        {
            LogWrite(string.Format(format, args), MsgLevel.Warn);
        }

        public void Info(string message)
        {
            LogWrite(message, MsgLevel.Infor);
        }

        public void Info(string format, params object[] args)
        {
            LogWrite(string.Format(format, args), MsgLevel.Infor);
        }
        ...
        public void Debug(string message)
        {
            if (isDebugEnabled)
            {
                LogWrite(message, MsgLevel.Debug);
            }
        }
        ...
        public void Debug(string format, params object[] args)
        {
            if (isDebugEnabled)
            {
                LogWrite(string.Format(format, args), MsgLevel.Debug);
            }
        }
    }
